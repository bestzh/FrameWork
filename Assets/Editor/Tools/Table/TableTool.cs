...
{
    public static string sourcePath = "Resources/Table";


    public static string tableCSPath = "Scripts/Table";
    public static string svnpath = "Assets/Resources/Table/";
    public static string resPath = Application.dataPath + "/" + sourcePath;


    public static string commandPath = Application.dataPath + "/Editor/Tools/Table/";

    /// <summary>
    /// Bin 文件结构
    /// int length 4 字节
    /// 长度为length byte 数组  代表基础类型
    /// 循环真实类型类型数据 
    /// </summary>
    //[MenuItem("Tools/表/二进制")]
    public static void TestGenBinaryTable()
    {
        GenBinaryTable(resPath);
    }
    public static void TestLoadBinaryTable()
    {
        string[] files = System.IO.Directory.GetFiles(Application.dataPath + "/Resources/TableBin", "*.bytes");
        foreach (var file in files)
        {
            FileStream fs = new FileStream(file, FileMode.Open);
            BinaryReader br = new BinaryReader(fs);
            int length = br.ReadInt32();
            List<TableType> headers = new List<TableType>();
            for (int i = 0; i < length; i++)
            {

                headers.Add((TableType)br.ReadByte());
    ...
    {
        string url = path.Replace("Table", "TableBin");

        FileUtilTool.DeleteFolder(path.Replace("Table", "TableBin"));
        ...
        foreach (var file in files)
        {
            ...
            string content = FileUtilTool.ReadFile(file);
            ...
            FileUtilTool.CreateFolderForFile(newfile);
    ...
    public static void AutoGenTableResAndScript()
    {
        ...
        AssetDatabase.Refresh();
        AssetDatabase.SaveAssets();
        Table.TableLoad.LoadFromResources();
    }
...
";
        ...
        string savePath = filePath.Replace(sourcePath, tableCSPath + "/Gen");
    ...
    public void GenerateCSharpClass_Handle(string filePath)
    {
        ...
        string savePath = filePath.Replace(sourcePath, tableCSPath + "/Handle");
        savePath = savePath.Replace(".csv", ".cs");
    ...
    public void GenerateTableLoad(string[] arrFileName)
    {
        ...
        string clear = "";
        string unload = "";
        string[] files = Directory.GetFiles(Application.dataPath + "/" + sourcePath);
        ...
        string savePath = Application.dataPath + "/" + tableCSPath + "/TableLoad.cs";
    ...
    {
        string filePath = Application.dataPath + "/Editor/Tools/Table/TableLoad.template";
        return File.ReadAllText(filePath);
    }

    public string GetCSharpClassTemplate()
    {
        string filePath = Application.dataPath + "/Editor/Tools/Table/Table.template";
        return File.ReadAllText(filePath);
    }

    public string GetCSharpClassTemplateHandle()
    {
        string filePath = Application.dataPath + "/Editor/Tools/Table/TableHandle.template";
        return File.ReadAllText(filePath);
    }
